...
#import <DPHue/WSLog.h>
#import "NSDictionary+Utilities.h"

@interface HARemoteHueLight ()
...
@implementation HARemoteHueLight
...
- (void)updateURLs {
  /*
  NSString *base = [NSString stringWithFormat:@"https://%@/api/%@/lights/%@",
                    self.host, self.username, self.number];
  */
  NSString *base = [NSString stringWithFormat:@"%@/user/sendMessageToBridge",
                  self.baseURL];
  // NSDictionary *payload = @{@"bridgeId" : self.bridgeId};
  _readURL = [NSURL URLWithString:base];
  _writeURL = [NSURL URLWithString:base];
...
- (void)write {
  ...
  NSDictionary *writeClip = @{
                              @"url" : [NSString stringWithFormat:@"/api/0/lights/%@/state", self.number],
                              @"method" : @"PUT",
                              @"body" : self.pendingChanges
  };
  
  NSDictionary *payload = @{@"bridgeId" : self.bridgeId, @"clipCommand" : writeClip};
  ...
  connection.completionBlock = ^(id obj, NSError *err) {
    ...
    NSString *resp = [[NSString alloc] initWithData:(NSData *)obj encoding:NSUTF8StringEncoding];
    NSLog(@"Write resp: %@", resp);
#endif
  };
  [connection start];
}
